    ...
    public void main()
    {
        ...
        do
        {
            ...
            if(ch>=1 && ch<=folders.length || (Character.isLetter(mch.charAt(0)) && Character.isLetter(mch.charAt(mch.length()-1))))
            {
                ...
                over = new ClassOver(mch,Double.parseDouble(t));
                System.out.println("Number of classes done : "+over.getHours());
                if(over.isOver())
                {
                    if(over.isMore(r,timer,mch))
                    {
                        double more = over.chancesOFAnyClass();
                        String what = (Math.abs(more)<=0 ? "has been":"will get");
                        System.out.println("Month class "+what+" "+(more<=0 ? "already over" : "over after")+" : "+(more<0  ? over.classCanBehappenend() : more));
                        sc.nextLine();
                        /*if(more<=0)
                            continue;*/
                    }
            ...
            if(over!=null)
            over.setRecordToShift(r);
            
            switch(ch)
            {
                case -355:
                System.out.println("\fSelect classname");
                for(int x=0;x<folders.length;x++)
                {
                    System.out.println((x+1)+" - Classes for "+folders[x]);
                }
            ...
            if(ch>=1 && ch<=folders.length || (Character.isLetter(mch.charAt(0)) && Character.isLetter(mch.charAt(mch.length()-1))))
            {
                ...
                if(over.objectChanged())
                {
                    record r1 = over.getRecord();
                    if(r1!=null)
                    {
                        r = r1;
                    }
