...
class ClassOver
{
    ...
    {
        analyse();
        boolean b =  (classHrs+givenClassHrs) > totalHRStoBeCompleted ;
        return b;
    }

    public boolean isMore(record record,Timer timer,String cname)
    {
        analyse();
        boolean b =  (classHrs+givenClassHrs) > totalHRStoBeCompleted ;
        if(Math.abs(chancesOFAnyClass()) > 0)
        {
            System.out.println("Press 1 to go let program change the class automatically");
            String n  = new Scanner(System.in).next();
            if(n.equalsIgnoreCase("1"))
            {
                shift = new Shift(chancesOFAnyClass(),givenClassHrs,record,dir,timer,cname);
                shift.start();
                objectChanged = true;
            }
        }
    ...
    public void setRecordToShift(record r)
    {
        if(shift!=null)
        shift.setrecord(r);
    }
    ...
    public double classCanBehappenend()
    {
        analyse();
        return totalHRStoBeCompleted - (classHrs);
    }
